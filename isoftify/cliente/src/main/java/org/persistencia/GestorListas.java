...
public class GestorListas {
	...
	public int crearLista(Lista l) {
		...
		try {
			String sql = String.format(
					"INSERT INTO LISTAS (cuenta, nombre, album, cancion) VALUES ('%s', '%s','%d','%d');", l.getCuenta(),
					l.getNombre_lista(), l.getAlbum(), l.getCancion());
			PreparedStatement stmt = ag.getStatement(sql);
			res = stmt.executeUpdate();
			stmt.close();
		} catch (Exception e) {
	...
	public Lista conseguirListaPorUsuario(String cuenta) {
		...
		try {
			...
			try (ResultSet res = stmt.executeQuery()) {
				while (res.next()) {
					lista = new Lista(res.getInt("id"), res.getString("nombre_lista"), res.getString("cuenta"),
							res.getInt("album"), res.getInt("cancion"));
				}
