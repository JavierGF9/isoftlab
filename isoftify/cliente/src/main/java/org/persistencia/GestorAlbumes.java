...
public class GestorAlbumes {
	...
	public int guardarAlbum(Album a) {
		...
		try {
			String sql = String.format(
					"INSERT INTO ALBUMES (nombre, autor, anio, duracion) VALUES ('%s', '%s', %d, '%s');",
					a.getNombre_album(), a.getAutor(), a.getAnio(), a.getDuracion());
	...
	public Album conseguirAlbumPorTitulo(String titulo) {
		...
		try {
			...
			try (ResultSet res = stmt.executeQuery()) {
				while (res.next()) {
					album = new Album(res.getInt("id"), res.getString("nombre"), res.getString("autor"),
							res.getString("duracion"), res.getInt("anio"));
				}
				res.close();
			} catch (Exception e) {
				e.printStackTrace();
	...
	public ArrayList<Album> conseguirTodosAlbumes() {
		...
		try {
			...
			try (ResultSet res = stmt.executeQuery()) {
				while (res.next()) {
					albumes.add(new Album(res.getInt("id"), res.getString("nombre"), res.getString("autor"),
							res.getString("duracion"), res.getInt("anio")));
				}
				res.close();
			} catch (Exception e) {
